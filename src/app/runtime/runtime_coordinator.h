...
//
// Extracted from src/main.cpp, where these behaviours lived in six connect()
// lambdas over three captured stack locals, which is why none of them could be
// stated, let alone tested.
//
// Contract: docs/module-api.md. This is a CONSUMER of
// MihomoBackend; it never reaches into the engine.
//
// WHAT THIS OWNS
...
//     quit. Those belong to app/lifecycle. The only handshake in this
//     direction is beginShutdown() / pruneAllSnapshots().
//   * proxy restoration during normal operation - RoutingController owns it.
...
class RuntimeCoordinator : public QObject, public core::backend::BackendObserver {
  ...
  public:
    ...
    // (BackendLifecycle::isManagedCoreActive, contract section 4) rather than
    ...
    // (contract section 5.1). That set is asked for, never re-derived.
    void pruneSnapshots();
    ...
    // The newest generation this consumer has observed (contract section 2).
  ...
  private:
    ...
    // The contract's section 2 consumer obligation. False when the event must
